"""
Telethon-based Accounts Router
Supports tdata sessions directly + OTP login.
"""
import asyncio
import uuid
import logging
...
from core.config import SESSIONS_DIR, BASE_DIR, TGDATA_DIR
...
async def login_start(req: LoginStartRequest, db: AsyncSession = Depends(get_db)):
    ...
    if req.proxy_id:
        ...
        # ALWAYS use a proxy for Pakistan - get first active proxy
...
async def _load_sessions_task(job_id: str, session_phones: list[str], proxies: list[dict]):
    """Background task: verify each tdata session and create Account records."""
    job = _load_jobs[job_id]
    job["status"] = "running"
    
    for i, phone in enumerate(session_phones):
        # ALWAYS use proxy for Pakistan
        proxy_dict = proxies[i % len(proxies)] if proxies else None
        if not proxy_dict and proxies:
            proxy_dict = proxies[0]  # Use first proxy if available
            ...
            async with async_session() as db:
                ...
                if existing:
                    ...
                    db.add(Log(
                        ...
                        message=f"tdata session loaded: {phone} ({name})",
                    ))
                    await db.commit()
...
async def load_sessions(background_tasks: BackgroundTasks, db: AsyncSession = Depends(get_db)):
    """Scan tgdata/ folder for tdata sessions and bulk-load them as accounts."""
    # Scan tdata folders
    if not TGDATA_DIR.exists():
        return {"job_id": None, "total": 0, "message": "tgdata folder not found"}
    
    tdata_folders = [f for f in TGDATA_DIR.iterdir() if f.is_dir() and (f / "tdata").exists()]
    if not tdata_folders:
        return {"job_id": None, "total": 0, "message": "No tdata folders found"}
    ...
    # Build list of new tdata phones
    session_phones = []
    for f in tdata_folders:
        stem = f.stem
        if stem not in existing_phones:
            session_phones.append(stem)
    
    if not session_phones:
        return {"job_id": None, "total": 0, "message": "All tdata sessions already loaded"}
...
async def check_health(account_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        # Get session path from tdata folder
        from pathlib import Path
        session_name = account.phone.replace('+', '').replace(' ', '')
        session_path = str(TGDATA_DIR / session_name / 'tdata')
        ...
        client = await client_manager.get_client(account.id, account.phone, account.proxy, session_path)
        me = await client.get_me()

        if me.restricted:
            account.status = "restricted"
            account.is_active = False
        elif me.scam or me.fake:
